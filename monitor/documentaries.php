...
$admin_section = 'documentaries';
$admin_page_title = 'وثائقيات / Documentaries';
...
if (!$category_id) {
    die('Category "documentaries" not found.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        header('Location: documentaries.php?msg=' . urlencode('Deleted.'));
        exit;
    }
    ...
    if ($title_ar && $title_fr && $final_url) {
        ...
        if ($edit_id) {
            ...
            header('Location: documentaries.php?msg=' . urlencode('Updated.'));
        } else {
            $slug = 'documentaries-' . time();
            ...
            header('Location: documentaries.php?msg=' . urlencode('Added.'));
        }
        exit;
...
<div class="mb-4">
  <a href="documentaries.php" class="btn btn-outline-secondary btn-sm">List</a>
  <?php if (!$editing): ?><a href="documentaries.php?add=1" class="btn btn-primary btn-sm">Add new</a><?php endif; ?>
</div>
...
<form method="post" enctype="multipart/form-data" class="card mb-4">
  <div class="card-body">
    ...
    <?php if ($editing): ?><a href="documentaries.php" class="btn btn-secondary">Cancel</a><?php endif; ?>
  </div>
</form>
<?php endif; ?>
...
<table class="table table-sm">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <a href="documentaries.php?edit=<?php echo (int)$i['id']; ?>" class="btn btn-sm btn-outline-primary">Edit</a>
