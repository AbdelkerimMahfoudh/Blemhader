...
$admin_section = 'meetings';
$admin_page_title = 'لقاءات / Meetings';

$stmt = $pdo->prepare('SELECT id FROM categories WHERE slug = ?');
$stmt->execute(['meetings']);
$cat = $stmt->fetch();
$category_id = $cat ? (int)$cat['id'] : 0;

if (!$category_id) {
    die('Category "meetings" not found.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete'])) {
        ...
        header('Location: meetings.php?msg=' . urlencode('Deleted.'));
        exit;
    }
    ...
    if ($title_ar && $title_fr && $final_url) {
        ...
        if ($edit_id) {
            ...
            header('Location: meetings.php?msg=' . urlencode('Updated.'));
        } else {
            $slug = 'meetings-' . time();
            ...
            header('Location: meetings.php?msg=' . urlencode('Added.'));
        }
        exit;
...
<div class="mb-4">
  <a href="meetings.php" class="btn btn-outline-secondary btn-sm">List</a>
  <?php if (!$editing): ?><a href="meetings.php?add=1" class="btn btn-primary btn-sm">Add new</a><?php endif; ?>
</div>
...
<form method="post" enctype="multipart/form-data" class="card mb-4">
  <div class="card-body">
    ...
    <?php if ($editing): ?><a href="meetings.php" class="btn btn-secondary">Cancel</a><?php endif; ?>
  </div>
</form>
<?php endif; ?>
...
<table class="table table-sm">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <a href="meetings.php?edit=<?php echo (int)$i['id']; ?>" class="btn btn-sm btn-outline-primary">Edit</a>
